...
function RoomTable(){
    ...
    function DeleteHandle(){
        const URL = `http://localhost:8080/admin/api/booking/` + idDelete;
    ...
    function handleStatusChange(e){
        ...
        axios({
            method: 'post',
            url: `http://localhost:8080/admin/api/booking/${id}/change_status`
        })
        .then(response=>{
    ...
    useEffect(()=>{
        fetch(`http://localhost:8080/admin/api/booking?page=${page}&sortField=${'id'}&sortDir=${'asc'}`)
        .then(res=>res.json())
        .then(
            (result)=>{
                setIsLoaded(true);
                ...
                bookings.map((booking, index)=>{
                    return(
                        <tr key={booking.id}>
                            ...
                            {booking.status?<FontAwesomeIcon onClick={handleStatusChange} icon="fas fa-check-circle" color="green" size="2x"/>:<FontAwesomeIcon onClick={handleStatusChange} icon="fas fa-check-circle" size="2x"/>}
                            </td>
