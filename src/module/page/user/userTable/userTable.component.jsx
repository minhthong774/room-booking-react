...
function UsersTable({searchText, triggerSearch}) {
    ...
    function DeleteHandle() {
        const URL = `${baseUrl}/admin/api/users/` + idDelete;
    ...
    function handleStatusChange(e) {
        ...
        axios({
            method: 'post',
            url: `${baseUrl}/admin/api/users/${id}/change_status`
        })
            .then(response => {
    ...
    useEffect(() => {
        fetch(`${baseUrl}/admin/api/users?page=${page}&sortField=${'id'}&sortDir=${'asc'}&searchText=${searchText}`)
            .then(res => res.json())
            .then(
                (result) => {
                    setIsLoaded(true);
                        ...
                        users.map((user, index) => {
                            return (
                                <tr key={user.id}>
                                    ...
                                    <td className="column2"><img src={baseUrl + user.avatarPath}
                                                                 alt="avatar"></img></td>
